...
class FormDefaultViewModel @Inject constructor(
  ...
  fun setForm(key: FormKey) {
    formPreferences = FormPreferences(application, key.eventId, key.formId)

    viewModelScope.launch(Dispatchers.IO) {
      try {
        val event = eventLocalDataSource.read(key.eventId)
        formJson = eventLocalDataSource.getForm(key.formId)?.json
        Form.fromJson(formJson)?.let { form ->
          val defaultForm = FormPreferences(application, event.id, form.id).getDefaults()
          _formState.postValue(FormState.fromForm(eventId = event.remoteId, form = form, defaultForm = defaultForm))
        }
